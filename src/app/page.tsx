...
const selectClass = "h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-slate-100 transition cursor-pointer shadow-sm";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}>
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="text-[10px] font-bold uppercase text-slate-400 block mb-2 tracking-widest">Description</label>
            <div className="text-sm text-slate-700 leading-relaxed max-h-60 overflow-y-auto bg-slate-50 p-4 rounded-2xl border border-slate-100 whitespace-pre-wrap">
  ...
  useEffect(() => {
    if (isOpen && initialData.ideas.length > 0) {
       ...
       setFormData({
         ...
         tags: [...gameNames, "Shorts"],
         hashtags: ["#shorts", ...gameNames.map(g => `#${g.replace(/\s+/g, '')}`)],
         status: "Draft"
       });
    }
  }, [isOpen, initialData]);
